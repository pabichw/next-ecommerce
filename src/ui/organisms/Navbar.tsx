...
function Navbar({ categories }: NavbarProps) {
  ...
  const linkTextCls = `hidden md:inline max-w-full overflow-x-hidden transition transition-[max-width] ${minified ? "!max-w-0" : ""
    }`;
  ...
  return (
    <div className="fixed top-0 z-10 bg-white shadow w-full flex align-center justify-between gap-8">
      <nav
        role="navigation"
        className={`column-wrapper mx-auto ${minified ? "py-[0.25rem]" : "py-2"
          } flex items-center justify-between w-full transition-[padding]`}
      >
        <div
          className={`scale-100 grayscale-0 transition transition-[transform,filter] ${minified ? "scale-[0.7] !grayscale" : ""
            }`}
        >
          <Logo />
        </div>
        <div className="flex gap-8 items-center">
          ...
          {categories?.map(category =>
            <ActiveLink className={linkCls} activeClassName={activeLinkCls} href={category.url as Route} exact={false}>
